# はじめに
2019/10/26開催のSalesforce女子部×Java女子部×JDKソムリエコラボ勉強会で実施する、Javaハンズオンの資料です。  
本ハンズオンでは、ご自分のマシンにSDKMAN!というツールを使ってJDK（Javaを動かすために必要なソフトウェアがまとまったもの)をインストールして、JShellというツールから実際にJavaを動かしてみることが目的です。  
  
事前準備やハンズオンの中でわからないことがあれば、お気軽にJava女子部スタッフにご質問ください！  

---
# 0.事前準備
---
## 0-1.Cygwinのインストール(Windowsのみ)
SDKMAN!はUnixベースのOS向けに作られているので、Windowsではそのままだと動きません。  
なので、Windowsを使う場合は事前にCygwinというWindows上でLinuxと同じようなコマンドを使えるようにするツールをインストールして、Cygwinを通してSDKMAN!を使います。  
Macの人は標準のターミナルでSDKMAN!を動かせるので、Cygwinのインストールは不要です。  

---
### Cygwinのダウンロード
下記のWebサイトからCygwinのインストーラーをダウンロードします。  
Cygwin https://cygwin.com  
  
ページ中程にある**Installing Cygwin**からインストーラーがダウンロードできます。  
ご自分のマシンのOSが64bit版の場合はsetup-x86_64.exe (64-bit installation)を、32bit版の場合はsetup-x86.exe(32-bit installation)をクリックしてダウンロードしてください。  
  
ご自分のPCのバージョンがどちらかわからない場合は、こちらのヘルプ等を参考にご確認ください。  
https://support.microsoft.com/ja-jp/help/15056/windows-32-64-bit-faq  

---
### Cygwinのインストール
ダウンロードしたインストーラーを起動してCygwinをインストールします。
- Download SourceはInstall from Internetを選んでください。
- Choose A Download SiteはどこでもOKです。とりあえず**.jp等の近いところを選んでおきましょう。
- ~~インストール先やパッケージは今回はデフォルトのままで大丈夫です。今後もCygwinを使うようであれば、エディタ等入れておくと便利かもしれません。~~
- Select Packageで、SDKMAN!を使用するために必要な**curl**、**zip**、**unzip**の3種類のパッケージをインストールしてください。Searchのテキストボックスにパッケージ名を入力して検索し、表示されたパッケージのSkipと書いてある箇所をダブルクリックするとバージョンが表示された状態になるので、その状態で「次へ」で進むとインストールされます。  
パッケージを選択せずにインストールを完了した場合は、再度インストーラーを起動して同じように操作（パッケージを選択してインストール）を行ってください。

---
### Cygwinの起動
デスクトップかスタートメニューのショートカットアイコン、またはインストール先のディレクトリに作成されたアイコンからCygwinを起動します。
黒いウィンドウが表示されたらOKです！

---
## 0-2.SDKMAN!のインストール(Win、Mac、Linux共通)
JDKをインストールしたり、使用するJDKのバージョンを切り替えたりするためのツール SDKMAN! https://sdkman.io をインストールします。

---
### SDKMAN!のインストール
Mac、Linux等を使用している方はターミナルを、Windowsを使用している方はCygwinを起動して、下記のコマンドを入力して実行します。
```
$ curl -s "https://get.sdkman.io" | bash
```
画面に完了(All done!)の表示が出たら、続けて下記のコマンドを実行します。
```
$ source "$HOME/.sdkman/bin/sdkman-init.sh"
```
インストールが成功したかチェックするためにsdkコマンドでバージョン表示してみましょう。
```
$ sdk version
```
SDKMAN 5.7.3+337 のようにバージョン番号が表示されれば準備OKです。お疲れ様でした！

---
# 1.SDKMAN!でJava(JDK)をインストールしてみよう
---
## 1-1.SDKMAN!でインストールできるJava(JDK)の種類を見てみる

実際にJava(JDK)をインストールする前に、SDKMAN!でインストールできるJava(JDK)の種類を確認してみましょう。
Windowsの方は事前準備でインストールしたCygwinを、Mac、Linux等を使用している方はターミナルを起動して、下記のSDKMAN!のコマンドを入力してみてください。
```
$ sdk list java
```
こんな感じの表が出てくると思います。
```
================================================================================
Available Java Versions
================================================================================
 Vendor        | Use | Version      | Dist    | Status     | Identifier
--------------------------------------------------------------------------------
```
この表に載っているものが、SDKMAN!でインストールできるJava(JDK)の種類です。  
提供元もバージョンもいっぱいあって何を選んだらいいのか迷っちゃいますね！  
JDKの特徴や選び方は後ほどソムリエと一緒に勉強することにして、今回はとりあえずこちらを使います。
```
 Java.net      |     | 13.0.1       | open    |            | 13.0.1-open 
```
Java.netのopen(Oracle OpenJDK)のというJDKの、バージョン13.0.1です。

listにこのJDKが表示されていない場合は、SDKMAN!のバージョンが古い可能性がありますので、下記のコマンドを実行してSDKMAN!をアップデートしてみてください。
```
$ sdk selfupdate force
```

---
## 1-2.SDKMANでJDKをインストールしてみる

Cygwinかターミナルで下記のコマンドを実行してJDKをインストールします。
```
$ sdk install java 13.0.1-open
```
完了のメッセージ（Done installing!）が出たらOKです！  

下記のようなメッセージ（インストールしたJDKをデフォルトにしますか？）が表示された場合は、問題なければ「Y」を入力してEnterを押してください。  
(既にご自分でJavaの環境を作っているなど、変更したくない場合は「n」を入力してEnterを押してください)
```
Do you want java 13.0.1-open to be set as default? (Y/n):
```

インストールができたので、Javaが使えるようになっているか下記のコマンドで確かめてみましょう。
```
$ java -version
```
openjdk version "13.0.1" のようにバージョンが表示されればOKです！

---
# 2.JShellでHelloWorld!してみよう
JDKがインストールできたので、実際にJavaを使ってみましょう！  
普通にJavaを動かす場合、Javaのコードを書いたファイルをコンパイルする必要がありますが、JShellというツールを使うとJavaのコードを対話的に実行することができます。

---
## 2-1.JShellを起動する
下記のコマンドを実行してJShellを起動しましょう。
```
$ jshell
```
JShellへようこそ、のメッセージが表示されて、行の先頭の表示が「jshell>」に変わっていればJShellが起動できています。
```
|  JShellへようこそ -- バージョン13.0.1
|  概要については、次を入力してください: /help intro

jshell> 
```

JShellを終了する場合は、/exitと入力してEnterを押します。
```
jshell> /exit
|  終了します
```

---
## 2-3.Javaを書いてみる（Hello World!)
それではJavaのコードを書いてみましょう！
下記のコードをJShellで入力して、Enterを押します。
```
jshell> System.out.print("Hello World!");
```
次の行にHello World!と表示されれば、Javaが実行できています。

```
jshell> System.out.print("Hello World!");
Hello World!
```
このコードでは、System.out.print()というメソッドを使って、標準出力にHello World!という文字を表示しました。  
(""は文字列を、;は行の終わりを表す記号です)  
  
Javaにはとてもたくさんのメソッドがあるので、ぜひ調べながら動かしてみてください！  
ちなみにJShellではコードを途中まで入力した状態でtabキーを押すと、入力の候補を表示したり、入力を補完してくれたりします。  

---
# EX.もうちょっとプログラムにチャレンジしてみる
## EX-1.メソッドを作って呼び出してみる
2-3では、Javaでもともと用意されているSystem.out.print()というメソッドを使ってHello World!を表示しました。  
今度はメソッド自体を自分で作ってみようと思います。  
まずは、サンプルとしてメソッドを書いたファイルを用意しましたので、ファイルを読み込んで実行してみましょう。  

[サンプルファイル](./multiplicationTable.java)  
こちらのリンク先のファイルをご自分のPCに保存してください。  
保存ができたら、JShellで以下のように/openコマンドを実行します。

```
jshell> /open <保存したファイルパス>
```
特にエラーなどが出なければOKです。  
JShellで/openコマンドを実行すると、指定したファイルの中身をJShellで1行ずつ書いたのと同じように実行してくれます。
このサンプルファイルには、九九の一覧表を出力する multiplicationTable() というメソッドが書いてあります。  
読み込んだメソッドを実行してみましょう。
```
jshell> multiplicationTable()
```
JShellに九九が表示されていればOKです！

---
## EX-2.プログラムを書いてみる(FizzBuzz)
プログラムの練習をするのに、よく使われるFizzBuzzというゲームがあります。  
1から順に数字を言っていって、3で割り切れるときは「Fizz」、5で割り切れるときは「Buzz」、両方で割り切れるときは「FizzBuzz」を数字の代わりに言わなければいけない、というルールで、プログラムでこれを実装すると下記のように出力されます。
```
1,
2,
Fizz,
4,
Buzz,
Fizz,
7,
8,
Fizz,
Buzz,
11,
Fizz,
13,
14,
FizzBuzz,
16,
・・・
```
さきほどサンプルファイルのメソッドを/openで読み込んでJShellで実行したのと同じように、FizzBuzzを実行するメソッドを書いてJShellで実行してみましょう。
ヒントが欲しい人は気軽に運営メンバーに質問してください！

---
## おまけ ちょっとだけコマンド紹介

ターミナルやCygwinを初めて使う人向けに、ちょっとだけ使い方（コマンド）を紹介します。

- ディレクトリの内容を表示する
```
$ ls
```

- 今いるディレクトリのパスを表示する
```
$ pwd
```

- ディレクトリを移動する
```
$ cd <ディレクトリ名>
```

- ファイルの内容を表示する
```
$ cat <ファイル名>
```

